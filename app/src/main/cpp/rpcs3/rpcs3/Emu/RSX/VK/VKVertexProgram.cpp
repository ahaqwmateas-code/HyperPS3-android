...
void VKVertexDecompilerThread::insertInputs(std::stringstream& OS, const std::vector<ParamType>& /*inputs*/)
{
    ...
    vk::glsl::program_input_type program_input_type;
    ...
    if(cfg_vertex_buffer_upload_mode_use_buffer_view()) {
        ...
        program_input_type=vk::glsl::input_type_texel_buffer;
    }
    else{
        OS
                << "layout(std430,set=0, binding=5)  readonly buffer persistent_input_stream_t{uint persistent_input_stream[];};\n"; // Data stream with persistent vertex data (cacheable)
        OS
                << "layout(std430,set=0, binding=6)  readonly buffer volatile_input_stream_t{uint volatile_input_stream[];} ;\n";   // Data stream with per-draw data (registers and immediate draw data)
        OS
                << "layout(std430,set=0, binding=7)  readonly buffer vertex_layout_stream_t{uvec2 vertex_layout_stream[];} ;\n";    // Data stream defining vertex data layout

        program_input_type=vk::glsl::input_type_storage_buffer;
    }
