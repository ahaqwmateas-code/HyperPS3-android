...
namespace vk
{
	...
	std::pair<VkDescriptorSetLayout, VkPipelineLayout> shader_interpreter::create_layout(VkDevice dev)
	{
		...
		CHECK_RESULT(_vkCreatePipelineLayout(dev, &layout_info, nullptr, &result));
	...
	void shader_interpreter::destroy()
	{
		...
		{
			_vkDestroyPipelineLayout(m_device, m_shared_pipeline_layout, nullptr);
			m_shared_pipeline_layout = VK_NULL_HANDLE;
		}

		if (m_shared_descriptor_layout)
		{
			_vkDestroyDescriptorSetLayout(m_device, m_shared_descriptor_layout, nullptr);
