...
namespace vk
{
	...
	fence::fence(VkDevice dev)
	{
		...
		CHECK_RESULT(_vkCreateFence(dev, &info, nullptr, &handle));
	}
	...
	fence::~fence()
	{
		if (handle)
		{
			_vkDestroyFence(owner, handle, nullptr);
		...
		: m_device(dev)
	{
		...
		CHECK_RESULT(_vkCreateSemaphore(m_device, &info, nullptr, &m_handle));
	}

	semaphore::~semaphore()
	{
		_vkDestroySemaphore(m_device, m_handle, nullptr);
	}
		...
		: m_device(&dev), m_domain(domain)
	{
		...
		CHECK_RESULT(_vkCreateEvent(dev, &info, nullptr, &m_vk_event));
	}
	...
	event::~event()
	{
		...
		{
			_vkDestroyEvent(*m_device, m_vk_event, nullptr);
		}
	}
	...
	void event::resolve_dependencies(const command_buffer& cmd, const VkDependencyInfoKHR& dependency)
	{
		...
		_vkCmdPipelineBarrier(cmd, src_stages, dst_stages, dependency.dependencyFlags,
	...
	void event::signal(const command_buffer& cmd, const VkDependencyInfoKHR& dependency)
	{
		...
		if (m_domain != sync_domain::host)
		{
			...
			{
				const auto dst_stages = v1_utils::gather_dst_stages(dependency);
				_vkCmdSetEvent(cmd, m_vk_event, dst_stages);
			}
		...
		if (m_backend != sync_backend::events_v2)
		{
			_vkCmdSetEvent(cmd, m_vk_event, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT);
			return;
		}
	...
	void event::host_signal() const
	{
		...
		{
			_vkSetEvent(*m_device, m_vk_event);
			return;
		}
	...
	void event::reset() const
	{
		if (m_backend != sync_backend::gpu_label) [[ likely ]]
		{
			_vkResetEvent(*m_device, m_vk_event);
			return;
		}
	...
	VkResult event::status() const
	{
		if (m_backend != sync_backend::gpu_label) [[ likely ]]
		{
			return _vkGetEventStatus(*m_device, m_vk_event);
		}
	...
	void gpu_label::signal(const vk::command_buffer& cmd, const VkDependencyInfoKHR& dependency)
	{
		...
		_vkCmdFillBuffer(cmd, m_buffer_handle, m_buffer_offset, 4, label_constants::set_);
	}
	...
	VkResult wait_for_fence(fence* pFence, u64 timeout)
	{
		...
		if (timeout)
		{
			return _vkWaitForFences(*g_render_device, 1, &pFence->handle, VK_FALSE, timeout * 1000ull);
		}
		else
		{
			while (auto status = _vkGetFenceStatus(*g_render_device, pFence->handle))
			{
				switch (status)
				{
				case VK_NOT_READY:
