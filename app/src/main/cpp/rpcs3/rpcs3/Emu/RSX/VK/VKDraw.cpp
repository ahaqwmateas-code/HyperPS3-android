...
void VKGSRender::update_draw_state()
{
	...
	_vkCmdPushConstants(*m_current_command_buffer, m_pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 32, 16, xx);
*/
	bind_viewport();
...
void VKGSRender::emit_geometry(u32 sub_index)
{
	...
	const auto old_persistent_buffer = m_persistent_attribute_storage ? m_persistent_attribute_storage->get_buffer() : null_buffer_view->get_buffer();
	const auto old_volatile_buffer = m_volatile_attribute_storage ? m_volatile_attribute_storage->get_buffer() : null_buffer_view->get_buffer();
	...
	auto persistent_buffer = m_persistent_attribute_storage ? m_persistent_attribute_storage->get_buffer() : null_buffer_view->get_buffer();
	auto volatile_buffer = m_volatile_attribute_storage ? m_volatile_attribute_storage->get_buffer() : null_buffer_view->get_buffer();
	...
	if (m_current_draw.subdraw_id == 0)
	{
		...
		if (vk::test_status_interrupt(vk::heap_changed))
		{
			if (m_vertex_layout_storage &&
				!m_vertex_layout_storage->is(m_vertex_layout_ring_info.heap->value))
			{
	...
	if (update_descriptors)
	{
        if(cfg_vertex_buffer_upload_mode_use_buffer_view()) {
            m_program->bind_uniform(std::get<0>(persistent_buffer), binding_table.vertex_buffers_first_bind_slot,
                                    m_current_frame->descriptor_set);
            m_program->bind_uniform(std::get<0>(volatile_buffer),
                                    binding_table.vertex_buffers_first_bind_slot + 1,
                                    m_current_frame->descriptor_set);
            m_program->bind_uniform(std::get<0>(m_vertex_layout_storage->get_buffer()),
                                    binding_table.vertex_buffers_first_bind_slot + 2,
                                    m_current_frame->descriptor_set);
        }
        else{
            m_program->bind_uniform(std::get<1>(persistent_buffer), binding_table.vertex_buffers_first_bind_slot,
                                    m_current_frame->descriptor_set);
            m_program->bind_uniform(std::get<1>(volatile_buffer),
                                    binding_table.vertex_buffers_first_bind_slot + 1,
                                    m_current_frame->descriptor_set);
            m_program->bind_uniform(std::get<1>(m_vertex_layout_storage->get_buffer()),
                                    binding_table.vertex_buffers_first_bind_slot + 2,
                                    m_current_frame->descriptor_set);
        }
	}
	...
	if (reload_state)
	{
		_vkCmdBindPipeline(*m_current_command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_program->pipeline);
