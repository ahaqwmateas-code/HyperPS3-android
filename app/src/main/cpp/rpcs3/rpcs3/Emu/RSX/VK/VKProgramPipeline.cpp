...
namespace vk
{
	namespace glsl
	{
		...
		program::~program()
		{
			_vkDestroyPipeline(m_device, pipeline, nullptr);
		}
		...
		void program::bind_uniform(const VkBuffer& buffer, u32 binding_point, vk::descriptor_set& set)
		{
			const VkDescriptorBufferInfo buffer_info = { buffer, 0, VK_WHOLE_SIZE };
			set.push(buffer_info, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, binding_point);
            attribute_location_mask |= (1ull << binding_point);
