	...
	namespace overlays
	{
		...
		extern void reset_debug_overlay()
		{
			...
			if (auto manager = g_fxo->try_get<rsx::overlays::display_manager>())
			{
				...
				if (g_cfg.video.debug_overlay || g_cfg.io.debug_overlay||g_cfg.misc.mem_debug_overlay)
				{
					if (!overlay)
					{
		...
		{
			if (!g_cfg.misc.use_native_interface || (!g_cfg.video.debug_overlay && !g_cfg.io.debug_overlay &&!g_cfg.misc.mem_debug_overlay))
				return;
