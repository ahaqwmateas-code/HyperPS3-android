...
namespace vk
{
	...
	struct depth_resolve_base : public overlay_pass
	{
		...
		{
			_vkCmdPushConstants(cmd, m_pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, static_parameters_width * 4, static_parameters);
		}
	...
	struct stencilonly_resolve : depth_resolve_base
	{
		...
		void emit_geometry(vk::command_buffer& cmd) override
		{
			...
			for (s32 write_mask = 0x1; write_mask <= 0x80; write_mask <<= 1)
			{
				...
				_vkCmdPushConstants(cmd, m_pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 8, 4, &write_mask);
